...
module.exports = ({ accessKeyId, secretAccessKey, bucket = 'jsreport', lock = { } }) => {
  if (!accessKeyId) {
  ...
  return {
    init: async () => {
      if (lock) {
        ...
        lock.region = 'eu-west-1'

        const sqs = new SQS({ accessKeyId: accessKeyId, secretAccessKey: secretAccessKey, region: lock.region })
        Promise.promisifyAll(sqs)

        const queueRes = await sqs.createQueueAsync(lock)
    ...
    releaseLock: (l) => lock ? sqs.deleteMessageAsync({ QueueUrl: queueUrl, ReceiptHandle: l.Messages[0].ReceiptHandle }) : null
  }
}
