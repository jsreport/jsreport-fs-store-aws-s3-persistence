  ...
  return {
    ...
    rename: async (p, pp) => {
      ...
      return Promise.all(objectsToRename.Contents.map(async (e) => {
        const newName = e.Key.replace(p.replace(/^\//, ''), pp.replace(/^\//, ''))
    ...
    releaseLock: (l) => l === 'initializing-lock' ? Promise.resolve() : sqs.deleteMessageAsync({ QueueUrl: queueUrl, ReceiptHandle: l.Messages[0].ReceiptHandle })
  }
}
